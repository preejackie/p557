...
def compute_sum(listOfTensors):
    for node in listOfTensors:
      output += node
    return output
...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
      sumOfAdjoints = sum_node_list(node_to_output_grads_list[node])
      node.grad = sumOfAdjoints
      if node.op is None:
        continue
      tup = node.op.gradient_as_tuple(sumOfAdjoints, node)
    
      i = 0
      for kv in node.inputs:
        if kv not in node_to_output_grads_list:
          node_to_output_grads_list[kv] = []
        node_to_output_grads_list[kv].append(tup[i])
        i = i+1
